...
class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    // ---- TOUCH / POINTER (toàn màn hình game = nhảy) ----
    this.input.on('pointerdown', (ptr) => {
      // Không nhảy khi đang ở overlay câu hỏi
      if (this.st === ST.QUESTION) return;
      // Vùng nhảy: nửa màn hình trái (tránh chạm nút chơi lại)
      if (ptr.y > H * 0.75 && ptr.x > W * 0.6) return;
      this.jumpBtnDown = true;
      if (this.st === ST.IDLE) {
    ...
    this.input.on('pointerup', () => { this.jumpBtnDown = false; });

    // ---- UI ----
    this.add.text(W/2, 14, '💕 Khoảng cách đến Crush', {
      fontSize:'14px', fill:'#f9a8d4', fontFamily:'Segoe UI'
    }).setOrigin(0.5,0).setDepth(10);
    this.add.rectangle(W/2, 40, W*.7, 14, 0x1e293b).setDepth(10);
    this.distBar = this.add.rectangle(W/2-W*.35, 40, W*.7, 14, 0xec4899)
      .setOrigin(0,0.5).setDepth(11);
    this.distBarShine = this.add.rectangle(W/2-W*.35, 36, W*.7, 5, 0xfce7f3, 0.4)
      .setOrigin(0,0.5).setDepth(12);

    this.scoreTxt = this.add.text(12,12,'Score: 0',{
      fontSize:'18px', fill:'#f1f5f9', fontFamily:'Segoe UI'
    }).setDepth(10);
    this.obsTxt = this.add.text(W-12,12,'🌵 0/5',{
      fontSize:'16px', fill:'#fbbf24', fontFamily:'Segoe UI'
    }).setOrigin(1,0).setDepth(10);
    this.failTxt = this.add.text(W-12,36,'🤍🤍🤍',{
      fontSize:'20px', fill:'#f43f5e', fontFamily:'Segoe UI'
    }).setOrigin(1,0).setDepth(10);

    // ---- NÚT NHẢY (mobile) — hiện góc phải dưới ----
    this.jumpBtn = this.add.text(W - 28, H - 28, '⬆️', {
      fontSize:'54px'
    }).setOrigin(1,1).setDepth(20).setAlpha(0.7).setInteractive({ useHandCursor:true });
    ...
    this.hintText = this.add.text(W/2, H/2,
      ...
      backgroundColor:'#0f172a', padding:{x:22,y:16}, align:'center'
  ...
  onObstacleCleared() {
    ...
    if (!nextBoss) { this.obsTxt.setText('🏁'); return; }
  ...
  triggerBossWarning() {
    ...
    const wt = this.add.text(W/2, H*.32, '⚠️  BOSS APPEARS!', {
      fontSize:'38px', fill:'#fde047', fontFamily:'Segoe UI', fontStyle:'bold',
      stroke:'#92400e', strokeThickness:5
    ...
    const nt = this.add.text(W/2, H*.50, boss.name, {
      fontSize:'28px', fill:'#f9a8d4', fontFamily:'Segoe UI', fontStyle:'bold'
    }).setOrigin(0.5).setDepth(19).setAlpha(0);
  ...
  spawnBoss() {
    ...
    const texKey = boss.id==='mini1' ? 'boss_mini1'
                 : boss.id==='mini2' ? 'boss_mini2' : 'boss_final';
  ...
  askQuestion() {
    ...
    // Delay focus để tránh lỗi trên iOS
    setTimeout(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (i < 14) {
        ...
        this.tweens.add({ targets:h, x:x+Phaser.Math.Between(-200,200), y:y+Phaser.Math.Between(-200,80),
          alpha:0, scale:Phaser.Math.FloatBetween(0.6,1.8), duration:Phaser.Math.Between(900,1600), ease:'Power2',
  ...
  triggerHitBack() {
    ...
    const tag=this.add.text(this.female.x,this.female.y-70,`😤 +${DIST_FAIL_PENALTY}!`,
      { fontSize:'28px',fill:'#fca5a5',fontFamily:'Segoe UI',fontStyle:'bold' }).setOrigin(0.5).setDepth(16);
  ...
  onDinoHit(dino, obstacle) {
    ...
    if (this.failCount>=MAX_FAIL) { this.stopObstacleTimer(); this.physics.pause(); this.time.delayedCall(600,()=>this.triggerLose()); }
  }

  updateDistBar() {
    const W=this.scale.width, pct=Math.max(0,this.distance/DISTANCE_INIT);
    this.distBar.setSize(W*.7*pct,14);
    this.distBarShine.setSize(W*.7*pct,5);
  }
  updateFailUI() {
  ...
  triggerWin() {
    ...
    this.time.delayedCall(1400,()=>{
      ...
      this.tweens.add({ targets:this.dino, x:this.female.x-95, duration:1200, ease:'Power3.easeOut',
        onComplete:()=>{
          ...
          this.time.delayedCall(500,()=>{
            ...
            const bh=this.add.text(cx,this.dino.y-80,'❤️',{fontSize:'180px'}).setOrigin(0.5).setDepth(14).setAlpha(0).setScale(0.1);
            ...
            this.showHearts(); this.createConfetti(90); this.createFireworks(8,W/2,H*.3);
            const pink=this.add.rectangle(W/2,H/2,W,H,0xff69b4,0).setDepth(8);
            this.tweens.add({ targets:pink, alpha:0.3, duration:1500 });
            this.time.delayedCall(900,()=>{
              const t1=this.add.text(W/2,H*.08,'💕 Bắt kịp Crush rồi! 💕',
                { fontSize:'44px',fill:'#fde68a',fontFamily:'Segoe UI',fontStyle:'bold',stroke:'#92400e',strokeThickness:5 }
              ).setOrigin(0.5).setDepth(15).setAlpha(0);
              this.tweens.add({ targets:t1, alpha:1, duration:700 });
              const t2=this.add.text(W/2,H*.20,`Score: ${Math.floor(this.score)} 🌟  |  Vượt ${BOSSES.length} boss!`,
                { fontSize:'22px',fill:'#86efac',fontFamily:'Segoe UI' }).setOrigin(0.5).setDepth(15).setAlpha(0);
              ...
              const btn=this.add.text(W/2,H*.88,'▶  Chơi lại',
                { fontSize:'24px',fill:'#fff',fontFamily:'Segoe UI',backgroundColor:'#6366f1',padding:{x:28,y:14} }
  ...
  triggerLose() {
    ...
    const t1=this.add.text(W/2,H*.32,'💔 Crush đã chạy xa...',
      { fontSize:'44px',fill:'#fca5a5',fontFamily:'Segoe UI',fontStyle:'bold',stroke:'#7f0000',strokeThickness:5 }
    ).setOrigin(0.5).setDepth(19).setAlpha(0);
    this.tweens.add({ targets:t1, alpha:1, duration:500 });
    const t2=this.add.text(W/2,H*.47,'Thất bại rồi 😭\nLần sau cố lên nhé!',
      { fontSize:'24px',fill:'#fecaca',fontFamily:'Segoe UI',align:'center' }
    ).setOrigin(0.5).setDepth(19).setAlpha(0);
    this.tweens.add({ targets:t2, alpha:1, duration:500, delay:300 });
    const btn=this.add.text(W/2,H*.65,'🔄 Thử lại',
      { fontSize:'24px',fill:'#fff',fontFamily:'Segoe UI',backgroundColor:'#dc2626',padding:{x:28,y:14} }
    ...
    for(let i=0;i<50;i++){
      this.time.delayedCall(i*65,()=>{
        const h=this.add.text(Phaser.Math.Between(30,W-30),H*.92,
          Phaser.Utils.Array.GetRandom(ems),{fontSize:`${Phaser.Math.Between(28,64)}px`}).setOrigin(0.5).setDepth(10);
